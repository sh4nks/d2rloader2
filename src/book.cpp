...
Book::Book(const QString &title, const QString &author, int year, double rating,
           QObject *parent)
    : QObject(parent), m_title(title), m_author(author), m_year(year),
      m_rating(rating) {}
...
double Book::rating() const { return m_rating; }
